...
- When customer says goodbye or thank you and ends the conversation, say a warm farewell and stop`;
}
...
- For dineIn orders: collect food items, party size, time, and name
- For pickup orders: collect food items and name ONLY — never ask for party size or time
- For delivery orders: collect food items, delivery address (word for word as customer says it), and name — never ask for party size or time
- Party size and time are ONLY needed for dineIn and regular table reservations
- Capture delivery address EXACTLY as the customer says it, do not change or interpret it
- If customer says goodbye, thank you, or ends conversation, return a warm farewell as response
...
function looksLikeGoodbye(text) {
  if (!text) return false;
  return /\b(bye|goodbye|thank you|thanks|that's all|no thanks|done|finished|nothing else)\b/i.test(text);
}
...
async function processLLMMessage(body, req) {
  ...
  // ── ORDER ALREADY CONFIRMED — handle goodbye gracefully ──
  if (orderDraft.status === "confirmed") {
    ...
    // Customer said something else after order confirmed — check if new request
  ...
  if (bookingFlowActive || orderFlowActive) {
    ...
    // Update order draft — deduplicated items
    ...
    if (orderExtracted.orderType && !orderDraft.orderType) {
    ...
    if (
      ...
      if (orderDraft.orderType === "delivery") {
        if (!orderDraft.deliveryAddress) return { response: aiResponse || "What is your delivery address?" };
        if (!draft.customerName) return { response: aiResponse || "And what name should I put the order under?" };
      }
      ...
      const existingOrder = await Order.findOne({ callId });
      if (existingOrder) {
        return { response: `Your order is already placed under ${draft.customerName || "your name"}. Is there anything else I can help you with?` };
      }
      ...
      // Clear order draft after saving
      await Call.updateOne(
        { _id: freshCall._id },
        {
          $set: {
  ...
  // ── GOODBYE DETECTION (outside flow) ─────────────────────
  if (looksLikeGoodbye(latestUserText)) {
